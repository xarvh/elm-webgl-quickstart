...
worldToPixelScale { pixelSize, minimumVisibleWorldSize } =
    let
        maxScaleX =
            pixelSize.width / minimumVisibleWorldSize.width

        maxScaleY =
            pixelSize.height / minimumVisibleWorldSize.height
    in
...
worldToPixel viewport worldPosition =
    ...
    in
    { left = pixelX + viewport.pixelSize.width / 2
    , top = 1 - pixelY + viewport.pixelSize.height / 2
    }
...
worldToCameraTransform viewport =
    let
        ...
        scaleX =
            2.0 * scale / viewport.pixelSize.width

        scaleY =
            2.0 * scale / viewport.pixelSize.height
    in
...
actualVisibleWorldSize viewport =
    let
        scale =
            worldToPixelScale viewport
    in
    { width = viewport.pixelSize.width / scale
    , height = viewport.pixelSize.height / scale
    }
...
onWindowResize msgConstructor =
    Browser.Events.onResize (\w h -> msgConstructor { width = toFloat w, height = toFloat h })
